...
const { default: SDKWACLient } = require('@sdkwa/whatsapp-api-client')
const ApiUtils = require('./api-utils')

class  SDKWA extends ApiClient {

  constructor(token, options, webhookResponse) {
    super(token, options, webhookResponse)

    this.sdkwa = new SDKWACLient({
      host: token.host,
  ...
  async getUpdates (timeout, limit, offset, allowedUpdates) {
    ...
    this.noty = await this.sdkwa.receiveNotification()
    if (!this.noty) {
      return []
    }
    ...
    return this.mapToMessageUpdate(incomingMsg)
  }

  mapToMessageUpdate(incomingMsg) {
  ...
  async deleteWebhook () {
    if (this.noty && this.noty.receiptId) {
      return this.sdkwa.deleteNotification(this.noty.receiptId);
    }
  }

  sendMessage (chatId, text, extra) {
    const textWithKeys = ApiUtils.keyEmulation(extra, text)
    return this.sdkwa.sendMessage({chatId: chatId, message: textWithKeys})
  }
  ...
  sendContact (chatId, phoneNumber, firstName, extra) {
    return this.sdkwa.sendContact({chatId: chatId, contact: {
  ...
  sendPhoto (chatId, photo, extra) {
    return this.sdkwa.sendFileByUrl({chatId: chatId, urlFile:  photo, fileName: 'photo'})
  }
  ...
  sendDocument (chatId, document, extra) {
    return this.sdkwa.sendFileByUrl({chatId: chatId, urlFile: document, fileName: 'document'})
  }

  sendAudio (chatId, audio, extra) {
    return this.sdkwa.sendFileByUrl({chatId: chatId, urlFile: audio, fileName: 'video'})
  }
  ...
  sendVideo (chatId, video, extra) {
    return this.sdkwa.sendFileByUrl({chatId: chatId, urlFile: video, fileName: 'video'})
  }
  ...
  sendVoice (chatId, voice, extra) {
    return this.sdkwa.sendFileByUrl({chatId:chatId, urlFile: voice, fileName: 'voice'})
  }
